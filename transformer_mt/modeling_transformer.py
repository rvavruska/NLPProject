...
class TransformerDecoderLayer(nn.Module):
    ...
    def forward(self, decoder_hidden_states, encoder_hidden_states, key_padding_mask=None):
        ...
        residual = x
        # 4. Cross-Attention
        x = self.cross_attention(x, kv=encoder_hidden_states, key_padding_mask=key_padding_mask)
...
class TransfomerEncoderDecoderModel(nn.Module):
    ...
    def _decode(self, encoder_hidden_states, decoder_input_ids, key_padding_mask):
        ...
        # YOUR CODE STARTS HERE
        decoder_emb = self.decoder_embeddings(decoder_input_ids)
        decoder_emb = self._add_positions(decoder_emb)
        for layer in self.decoder_layers:
            decoder_emb = layer(decoder_hidden_states = decoder_emb, encoder_hidden_states=encoder_hidden_states, key_padding_mask = key_padding_mask)
        logits = self.out_proj(decoder_emb)
        ## YOUR CODE ENDS HERE
        return logits
