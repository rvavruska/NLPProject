...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, q, kv=None, key_padding_mask=None, return_attention=False):
        ...
        # q = bs, query_seq_len, hidden]
        # k = [bs, kv_seq_len, hidden]
        # v = [bs, kv_seq_len, hidden]
        if kv is not None:
            q, k, v = self.q(q), self.k(q), self.v(q)
        else:
        ...
        if key_padding_mask is not None:
            ...
            scores = scores.view(bs, self.num_heads, attending_seq, -1)
            scores.masked_fill_((key_padding_mask == 1).unsqueeze(1).unsqueeze(2), float("-inf"))
            scores = scores.reshape(bs * self.num_heads, attending_seq, -1)
            # YOUR CODE ENDS HERE
